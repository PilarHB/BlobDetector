...
def calculate_accuracy(worldPoints, imagePoints, total_points_used):
...
def from_3d_to_2d(im, world_coordinates, draw):
    # load camera calibration
    savedir = "..\\camera_data\\"
    dist = np.load(savedir + 'dist.npy')
    newcam_mtx = np.load(savedir + 'newcam_mtx.npy')
...
def setup_camera(savedir='..\\camera_data\\', display=False, writeValues=True):
    cam_mtx, dist, roi, newcam_mtx, inverse_newcam_mtx = load_parameters(savedir, display)
    # load center points from New Camera matrix
